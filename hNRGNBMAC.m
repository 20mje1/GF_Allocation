...
%                     %PDCCH Modling
%                 if pktInfo.RNTI ==3 || pktInfo.RNTI==4
%                     if pktInfo.RNTI ==4
%                     totalPRBs=totalPRBs-GetPRBSFcn(obj,grant.RNTI,grant.MCS);
%                     disp("pktInfo.RNTI")
%                     pktInfo.RNTI
%                     disp("grant.MCS")
%                     grant.MCS
%                     disp("GetPRBSFcn(obj,grant.RNTI,grant.MCS")
%                     GetPRBSFcn(obj,grant.RNTI,grant.MCS)
%                     if(totalPRBs>=0)
...
%                 else
    ...
    methods (Access = private)
        ...
        function rxRequestToPhy(obj, rnti, uplinkGrant)
            ...
            function PRBs =GetPRBSFcn(obj,RNTI, MCS)

            if (MCS>=0 && MCS<=8)
            PRBs=24;
            elseif (MCS>=9 && MCS<=17)
            PRBs=12;
            elseif (MCS>=18 && MCS<=27)
            PRBs=6;
            end
        end
    end
