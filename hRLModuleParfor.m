classdef hRLModuleParfor < handle
    properties
    ...
    methods
        function obj = hRLModuleParfor ()
            ...
            obj.finalObject.Q = initializeValues(obj); 
            ...
            % run the simulation
            obj.finalObject.meanRewards=[];
            for episode=1: obj.total_episodes
                runSimulation(obj);
                if ~(isempty(obj.state))
                parfor stepOfepisode=1: obj.max_steps
                    temp=struct();
                    temp.final=[];
                    temp.currentState= obj.state;
                    currentStateIndex = find(sum(obj.state_space==temp.currentState,2)==obj.numberOfUEs);
                    temp.currentAction=obj.chooseAction(currentStateIndex);
                    currentActionIndex = find(sum(obj.action_space==temp.currentAction,2)==obj.numberOfUEs);
                    x=['Current State', num2str(temp.currentState,[1 1 1 1])];
                    disp(x)
                     %if (~isequal(temp.currentState,[1 1 1 1])) && (temp.stepOfepisode<=temp.max_steps) 
%                        if stepOfepisode==1 && episode==1
%                            tic
%                        end 
                       x=['step: ',num2str(stepOfepisode), ' of episode:',num2str(episode)];
                       disp(x);
                       temp.nextState = obj.state;
                       nextStateIndex = find(sum(obj.state_space==temp.nextState,2)==obj.numberOfUEs); 
                       reward= obj.getReward();
                       temp.nextAction= obj.chooseAction(nextStateIndex);
                       nextActionIndex = find(sum(obj.action_space==temp.nextAction,2)==obj.numberOfUEs);
                       ...
                       currentStateIndex = find(sum(obj.state_space==temp.currentState,2)==obj.numberOfUEs);
                       currentActionIndex = find(sum(obj.action_space==temp.currentAction,2)==obj.numberOfUEs);
                       temp.stepOfepisode=obj.stepOfepisode+1;

%                     if stepOfepisode==obj.max_steps && episode==1
%                         temp.Elapsed_time = toc
%                     end 
                    if stepOfepisode==obj.max_steps
                        temp.final(end+1)= mean(obj.episodeRewards(1,:));
                        obj.recordAwards(temp.final);
                        temp.episodeRewards=[];
                        temp.currentepisode=episode+1;

                    end
% 
%                     if episode==obj.total_episodes
% 
%                         temp.final
%                         temp.Elapsed_time
%                         temp.listOfActions
...
%                     end
                     ...
                     temp.epsilon = obj.min_exploration_proba + (obj.epsilon - obj.min_exploration_proba) * exp(-1.0 * episode * obj.exploration_decreasing_decay)
                     obj.updateEpsilon(temp.epsilon);
                     runSimulation(obj);
        ...
        function updateEpsilon(obj,temp)
            obj.epsilon=temp;
...
%            QNextstate = finalObject.Q(nextStateIndex,nextActionIndex);
%            obj.E(currentStateIndex,currentActionIndex) = obj.E(currentStateIndex,currentActionIndex)+1; 
%            delta = reward(1)+(obj.gamma*QNextstate) - finalObject.Q(currentStateIndex,currentActionIndex); %error delta
%            finalObject.Q = finalObject.Q + obj.alpha*delta.*obj.E; %update Q for all pairs
...
%         end
 ...
 function update(obj,currentStateIndex,nextStateIndex,currentActionIndex,nextActionIndex,reward) 
     ...
     obj.finalObject.Q(currentStateIndex, currentActionIndex) = (1-obj.alpha) * obj.finalObject.Q(currentStateIndex, currentActionIndex) +obj.alpha *(reward + obj.gamma*max(obj.finalObject.Q(nextStateIndex,:)));
 end 
 
 function recordAwards(obj,temp)
     
     obj.finalObject.meanRewards(end+1)=temp;
 end 
        ...
        function chosenAction = chooseAction(obj, state)
            ...
            if(randN > obj.epsilon) %greedy
                row=finalObject.Q(state,:);
                [maxQ,index] = max(row(row<0)); %get next pos
